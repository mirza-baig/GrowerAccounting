...
@Injectable()
export class InvoiceBatchListService {
  ...
  getInvoiceBatches(): Observable<IApinvoicePaymentRequestBatch[]> {
    const url = `${this.baseUrl}/Invoice/GetAPInvoicePaymentRequestBatches`;
    ...
    return this.http
      ...
      .map(response => {
        ...
        // }
        return <IApinvoicePaymentRequestBatch[]>response.json();
      })
      .catch(this.handleError);
  }

  /** post an invoice batch */
  postInvoiceBatch(batch: IApinvoicePaymentRequestBatch): Observable<IApiResponse> {
