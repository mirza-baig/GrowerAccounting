import { Injectable } from '@angular/core';
import { Http, Response } from '@angular/http';
import { Observable } from 'rxjs/Observable';
import 'rxjs/add/operator/do';
import 'rxjs/add/operator/catch';
import 'rxjs/add/observable/throw';
import 'rxjs/add/operator/map';
import 'rxjs/add/observable/of';
import { environment } from '../../../environments/environment';
import { _throw } from 'rxjs/observable/throw';
import { IApiResponse } from '../../models/api-response.interface';
import { AuthorizationUtilities } from 'src/app/shared/auth-utilities';
import { IApinvoicePaymentRequest } from 'src/app/models/ap-invoice-payment-request.interface';
import { IApinvoicePaymentRequestBatch } from 'src/app/models/ap-invoice-payment-request-batch.interface';
import { IWorkflowContact } from 'src/app/models/workflow-contact.interface';



@Injectable()
export class WorkflowContactManagementService {
  /** Base URL for the Grower API. */
  private baseUrl = environment.apiUrl;

  constructor(
    private http: Http,
    private _authUtil: AuthorizationUtilities
  ) {}

  /** get an list of workflow contacts */
  getWorkflowContacts(): Observable<IWorkflowContact[]> {
    const url = `${this.baseUrl}/Workflow/GetWorkflowContacts`;
    const options = this._authUtil.getWildcardHeaderOptions();

    return this.http
      .get(url, options)
      .map(response => {
        const resp = response.json();
        return <IWorkflowContact[]>resp;
      })
      .catch(this.handleError);
  }

  /** activate an approver and deactivate another */
  activateApproverForStage(id: number): Observable<IApiResponse> {
    const url = `${this.baseUrl}/Workflow/ActivateApproverForStage?id=` + id;
    const options = this._authUtil.getWildcardHeaderOptions();

    return this.http
      .get(url, options)
      .map(response => {
        const resp = response.json();
        return <IApiResponse>resp;
      })
      .catch(this.handleError);
  }

  /** Post a workflow contact */
  postWorkflowContact(contact: IWorkflowContact): Observable<IApiResponse> {
    const url = `${this.baseUrl}/Workflow/PostWorkflowContact`;
    const options = this._authUtil.getWildcardHeaderOptions();

    return this.http
      .post(url, contact, options)
      .map(response => {
        const resp = response.json();
        return <IApiResponse>resp;
      })
      .catch(this.handleError);
  }
 
  private handleError(error: Response): Observable<any> {
    const msg = error.toString();
    return _throw(msg);
  }

}

