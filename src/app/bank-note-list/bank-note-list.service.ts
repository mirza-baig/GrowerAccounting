...
@Injectable()
export class BankNoteListService {
  ...
  /** get the list of bank notes for a grower */
  getBankNotesByGrowerId(id: number): Observable<IGrowerBankNote[]> {
    const url = `${this.baseUrl}/BankNote/GetBankNotesByGrowerId?id=` + id;
    const options = this._authUtil.getWildcardHeaderOptions();
  ...
  /** delete a matching grower bank note */
  deleteBankNote(id: number): Observable<any> {
    const url = `${this.baseUrl}/BankNote/DeleteBankNote?id=` + id;
    const options = this._authUtil.getWildcardHeaderOptions();
